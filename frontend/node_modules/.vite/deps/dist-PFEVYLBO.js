import {
  html
} from "./chunk-VABMP3P7.js";
import "./chunk-XQGIIDLM.js";
import "./chunk-A7F75H44.js";
import {
  ExternalTokenizer,
  LRParser,
  LocalTokenGroup
} from "./chunk-KMJKMOSE.js";
import "./chunk-PZKHRYRD.js";
import {
  LRLanguage,
  LanguageSupport,
  delimitedIndent,
  ...
  for (; ; ) {
    ...
    if (next == 123) {
      ...
      if (after == 123) {
        ...
        if (input.pos > start)
          break;
        input.acceptToken(commentTagStart, 2);
        return;
      } else if (after == 37) {
        if (input.pos > start)
          break;
...
var spec_identifier = { __proto__: null, in: 38, is: 40, and: 46, or: 48, not: 52, if: 78, else: 80, true: 98, false: 98, self: 100, super: 102, loop: 104, recursive: 136, scoped: 160, required: 162, as: 256, import: 260, ignore: 268, missing: 270, with: 272, without: 274, context: 276 };
var spec_TagName = { __proto__: null, if: 112, elif: 118, else: 122, endif: 126, for: 132, endfor: 140, raw: 146, endraw: 152, block: 158, endblock: 166, macro: 172, endmacro: 182, call: 188, endcall: 192, filter: 198, endfilter: 202, set: 208, endset: 212, trans: 218, pluralize: 222, endtrans: 226, with: 232, endwith: 236, autoescape: 242, endautoescape: 246, import: 254, from: 258, include: 266 };
var parser = LRParser.deserialize({
  ...
  tokenizers: [base, raw, 1, 2, 3, 4, 5, new LocalTokenGroup("b~RPstU~XP#q#r[~aO$Q~~", 17, 173)],
  topRules: { "Template": [0, 5] },
  specialized: [{ term: 161, get: (value) => spec_identifier[value] || -1 }, { term: 55, get: (value) => spec_TagName[value] || -1 }],
  tokenPrec: 3602
});
function completions(words, type) {
  return words.split(" ").map((label) => ({ label, type }));
}
var Filters = completions("abs attr batch capitalize center default dictsort escape filesizeformat first float forceescape format groupby indent int items join last length list lower map max min pprint random reject rejectattr replace reverse round safe select selectattr slice sort string striptags sum title tojson trim truncate unique upper urlencode urlize wordcount wordwrap xmlattr", "function");
var Functions = completions("boolean callable defined divisibleby eq escaped even filter float ge gt in integer iterable le lower lt mapping ne none number odd sameas sequence string test undefined upper range lipsum dict joiner namespace", "function");
var Globals = completions("loop super self true false varargs kwargs caller name arguments catch_kwargs catch_varargs caller", "keyword");
var Expressions = Functions.concat(Globals);
var Tags = completions("raw endraw filter endfilter trans pluralize endtrans with endwith autoescape endautoescape if elif else endif for endfor call endcall block endblock set endset macro endmacro import include break continue debug do extends", "keyword");
function findContext(context) {
  ...
  if (word)
    return { type: "expr", from: word.from };
  if (context.explicit)
    return { type: "expr" };
  return null;
}
...
function jinjaCompletionSource(config = {}) {
  ...
  return (context) => {
    ...
    if (cx.type == "filter")
      ...
      options = tags2;
    else if (cx.type == "expr")
      options = exprs;
    else
      options = properties ? resolveProperties(context.state, cx.target, context, properties) : [];
...
function statementIndent(except) {
  return (context) => {
...
var tagLanguage = LRLanguage.define({
  ...
  parser: parser.configure({
    props: [
      styleTags({
        "TagName raw endraw filter endfilter as trans pluralize endtrans with endwith autoescape endautoescape": tags.keyword,
        "required scoped recursive with without context ignore missing": tags.modifier,
        "self": tags.self,
        "loop super": tags.standard(tags.variableName),
        "if elif else endif for endfor call endcall": tags.controlKeyword,
        "block endblock set endset macro endmacro import from include": tags.definitionKeyword,
        "Comment/...": tags.blockComment,
        "VariableName": tags.variableName,
        "Definition": tags.definition(tags.variableName),
        "PropertyName": tags.propertyName,
        "FilterName": tags.special(tags.variableName),
        "ArithOp": tags.arithmeticOperator,
        "AssignOp": tags.definitionOperator,
        "not and or": tags.logicOperator,
        "CompareOp": tags.compareOperator,
        "in is": tags.operatorKeyword,
        "FilterOp ConcatOp": tags.operator,
      ...
      foldNodeProp.add({
        "Statement Comment"(tree) {
          ...
          if (!first || first.name != "Tag" && first.name != "{#")
            return null;
...
var jinjaLanguage = makeJinja(baseHTML.language);
function jinja(config = {}) {
  let base2 = config.base || baseHTML;
  let lang = base2.language == baseHTML.language ? jinjaLanguage : makeJinja(base2.language);
...
//# sourceMappingURL=dist-PFEVYLBO.js.map
